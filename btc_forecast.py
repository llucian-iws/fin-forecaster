...
try:
    ...
    # Model leg: Garman-Klass-fed HAR-RV, the construct validated on the
    # 300-fold lite backtest (ROADMAP 2.2: coverage 0.890 in-window and
    # closest to nominal, band 10% narrower than the GARCH leg, width std
    # -15%). Falls back to the GARCH forecast when the HAR fit is degenerate.
    # Blended with implied (DVOL) when available; both legs are annualized %,
    # so divide by ANN_HOURLY to get an hourly log-return std.
    _gkv = volatility.gk_variance(df['Open'].values, df['High'].values,
                                  df['Low'].values, df['Close'].values)
    _hfg = volatility.har_rv_forecast(df['log_ret'].values,
                                      horizon=max(1, int(round(hours_to_target))),
                                      ann=ANN_HOURLY, rv=_gkv)
    if _hfg is not None:
        model_leg_pct, model_leg_name = _hfg * 100.0, 'GK-HAR'
    else:
        model_leg_pct, model_leg_name = vol_info['forecast_rv'], 'GARCH (HAR fallback)'
    fwd_components = [model_leg_pct]
    ...
    vol_shock_source = (
        f"forward model [{model_leg_name} {model_leg_pct:.1f}%"
...
# HEADLINE band: the validated single forward-vol shock at the shrunk model
# drift. The regime-composite band (probability-weighted aggregate of the
# scenarios) was REMOVED after failing validation on every backtest run
# (lite150 CRPS DM p=0.002, cnn20 replicated, lite300 p<0.001; over-covers at
# 2x the width) — regime drifts inject dispersion, not information. The
# scenarios below remain as illustrative per-regime views only.
...
print(f"\n  ╭─ HEADLINE 90% BAND (validated forward-vol shock) ─╮")
...
with open(report_file, 'w') as f:
    ...
    f.write(f"\n\nHEADLINE 90% BAND (validated single forward-vol shock at the shrunk drift):\n")
    f.write(f"  Shock source: {vol_shock_source}\n")
